...
import { Loader2, CheckCircle, XCircle, GraduationCap, School, User, Mail, Lock, MapPin, Building2, Eye, EyeOff } from "lucide-react";
import Link from "next/link";

// 1. ROLE SELECTION (STEP 0)
export const Step0Role = ({ setRole, setStep, t }: any) => (
  <div className="grid gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
    <button onClick={() => { setRole("student"); setStep(1); }} className="group p-5 md:p-6 rounded-3xl border border-slate-200 bg-white hover:border-blue-500 hover:shadow-[0_8px_30px_rgb(59,130,246,0.1)] transition-all text-left flex items-center gap-5">
      <div className="w-14 h-14 bg-emerald-50 rounded-2xl flex items-center justify-center text-emerald-600 shadow-sm group-hover:scale-110 group-hover:-rotate-3 transition-transform">
        <GraduationCap size={28} />
      </div>
      <div>
        <h3 className="text-[17px] font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{t.roles.student.title}</h3>
        <p className="text-[14px] font-medium text-slate-500 mt-0.5">{t.roles.student.desc}</p>
      </div>
    </button>

    <button onClick={() => { setRole("teacher"); setStep(1); }} className="group p-5 md:p-6 rounded-3xl border border-slate-200 bg-white hover:border-blue-500 hover:shadow-[0_8px_30px_rgb(59,130,246,0.1)] transition-all text-left flex items-center gap-5">
      <div className="w-14 h-14 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-600 shadow-sm group-hover:scale-110 group-hover:rotate-3 transition-transform">
        <School size={28} />
      </div>
      <div>
        <h3 className="text-[17px] font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{t.roles.teacher.title}</h3>
        <p className="text-[14px] font-medium text-slate-500 mt-0.5">{t.roles.teacher.desc}</p>
      </div>
    </button>
    ...
    <div className="mt-6 text-center">
      <p className="text-sm font-medium text-slate-500">
        ...
        <Link href="/auth/login" className="text-blue-600 font-bold hover:underline ml-1">
...
export const Step1Credentials = ({ formData, handleChange, usernameError, usernameAvailable, isCheckingUser, showPassword, setShowPassword, t }: any) => (
  <div className="space-y-4 animate-in fade-in slide-in-from-right">
    ...
    <div className="relative group">
      ...
      <input name="username" type="text" placeholder={t.inputs.username} required value={formData.username} onChange={handleChange} className={`w-full pl-12 pr-10 py-3.5 rounded-xl border outline-none font-semibold text-[15px] transition text-slate-900 bg-slate-50 placeholder:text-slate-400 focus:bg-white focus:ring-4 ${usernameError ? "border-red-300 focus:border-red-500 focus:ring-red-500/10" : usernameAvailable === true ? "border-green-300 focus:border-green-500 focus:ring-green-500/10" : usernameAvailable === false ? "border-amber-300 focus:border-amber-500 focus:ring-amber-500/10" : "border-slate-200 focus:border-blue-500 focus:ring-blue-500/10"}`} />
...
export const Step2Personal = ({ formData, handleChange, t }: any) => (
  <div className="space-y-4 animate-in fade-in slide-in-from-right">
    <div className="group">
      <input name="fullName" type="text" placeholder={t.inputs.fullname} required value={formData.fullName} onChange={handleChange} className="w-full px-4 py-3.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-900 placeholder:text-slate-400 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10 outline-none font-semibold transition text-[15px]" />
    </div>
    <div className="grid sm:grid-cols-2 gap-4">
      <div>
        <label className="text-[11px] font-bold text-slate-500 uppercase ml-1 mb-1.5 block">{t.inputs.birth}</label>
        <input name="birthDate" type="date" required max={new Date().toISOString().split("T")[0]} value={formData.birthDate} onChange={handleChange} className="w-full px-4 py-3.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-900 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10 outline-none font-semibold transition text-[15px]" />
      </div>
      <div>
        <label className="text-[11px] font-bold text-slate-500 uppercase ml-1 mb-1.5 block">{t.inputs.phone}</label>
        <input name="phone" type="tel" required value={formData.phone} onChange={handleChange} placeholder="+998" className="w-full px-4 py-3.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-900 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10 outline-none font-semibold transition text-[15px]" />
      </div>
    </div>
  </div>
);
...
export const Step3Location = ({ formData, setFormData, handleChange, locations, t }: any) => (
  <div className="space-y-4 animate-in fade-in slide-in-from-right">
    ...
    <select name="region" value={formData.region} required onChange={(e) => setFormData({ ...formData, region: e.target.value, district: "" })} className={`w-full px-4 py-3.5 rounded-xl border bg-slate-50 text-slate-900 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10 outline-none font-semibold text-[15px] transition ${formData.region ? "border-slate-200" : "border-slate-200"}`}>
      <option value="">{t.selects.region}</option>
      {Object.keys(locations).map((r) => <option key={r} value={r}>{r}</option>)}
    </select>
    <select name="district" value={formData.district} required onChange={handleChange} disabled={!formData.region} className={`w-full px-4 py-3.5 rounded-xl border bg-slate-50 text-slate-900 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10 outline-none font-semibold text-[15px] transition disabled:opacity-50 disabled:bg-slate-100 ${formData.district ? "border-slate-200" : "border-slate-200"}`}>
...
export const Step4WorkEdu = ({ formData, handleChange, role, t }: any) => (
  <div className="space-y-4 animate-in fade-in slide-in-from-right">
    ...
    {role === "student" ? (
      <select name="gradeLevel" value={formData.gradeLevel} onChange={handleChange} required className="w-full px-4 py-3.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-900 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10 outline-none font-semibold text-[15px] transition">
      ...
      <select name="schoolSubject" required value={formData.schoolSubject} onChange={handleChange} className="w-full px-4 py-3.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-900 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10 outline-none font-semibold text-[15px] transition">
