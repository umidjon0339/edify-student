...
export default function TeacherSignupFlow({ onBack, t, locations }: any) {
  ...
  const [formData, setFormData] = useState({
    ...
    phone: "+998 ", country: "Uzbekistan", region: "", district: "", institutionName: "", schoolSubject: ""
  });
  ...
  const handleGoogleFinalize = async () => {
    ...
    if (!formData.schoolSubject || !formData.gender) return toast.error(t.validation.fillAll);
    ...
    try {
      ...
      const newProfile = {
        ...
        phone: null, birthDate: null, gender: formData.gender, role: "teacher", institution: null,
        location: { country: "Uzbekistan", region: null, district: null },
        grade: "Teacher", subject: formData.schoolSubject, verifiedTeacher: false, experience: 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (step === 4) {
      if (!formData.institutionName || !formData.schoolSubject) return toast.error(t.validation.fillAll);
      await handleSignup();
      return;
    }
    if (step === 10) {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      const newProfile = {
        ...
        gender: formData.gender, role: "teacher", institution: formData.institutionName,
      ...
      toast.success(t.validation.welcome.replace("{role}", "Teacher"));
      router.push("/teacher/dashboard");
    } catch (error: any) {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full">
      ...
      {step === 4 && <Step4WorkEdu formData={formData} handleChange={handleChange} role="teacher" t={t} />}
      {step === 10 && <Step10Google formData={formData} handleChange={handleChange} googleUser={googleUser} role="teacher" usernameError={usernameError} usernameAvailable={usernameAvailable} isCheckingUser={isCheckingUser} t={t} />}
