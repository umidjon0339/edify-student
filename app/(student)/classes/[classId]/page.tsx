...
import { 
  ...
  User, Hash, Calendar, Folder, Loader2, AlertTriangle, Trophy, Info, X
} from 'lucide-react';
...
const CLASS_TRANSLATIONS: any = {
  uz: {
    ...
    tabs: { assignments: "Topshiriqlar", grades: "Baholarim", leaderboard: "Reyting", materials: "Materiallar" },
  ...
  en: {
    ...
    tabs: { assignments: "Assignments", grades: "Grades", leaderboard: "Leaderboard", materials: "Materials" },
  ...
  ru: {
    ...
    tabs: { assignments: "Задания", grades: "Оценки", leaderboard: "Рейтинг", materials: "Материалы" },
...
export default function StudentClassPage() {
  ...
  // 🟢 Replaced Leave Modal with Info Modal
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-6 md:pt-10">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3 sm:gap-4">
            <Link href="/classes" className="w-10 h-10 sm:w-12 sm:h-12 bg-white border-2 border-zinc-200 rounded-xl flex items-center justify-center text-zinc-500 hover:text-indigo-600 hover:border-indigo-200 transition-all shadow-sm active:scale-95">
              <ChevronLeft size={20} strokeWidth={3} />
            </Link>
            
            <h1 className="text-[20px] sm:text-[24px] font-black text-zinc-900 tracking-tight truncate max-w-[180px] sm:max-w-sm md:max-w-md">
              {classData.title}
            </h1>
        ...
        {/* 🟢 TABS (Starts immediately below header) */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 p-1.5 bg-zinc-200/80 rounded-[1.25rem] mb-6 shadow-sm border-2 border-white/50 sticky top-0 z-40 backdrop-blur-xl">
          {[
            ...
            return (
               <button 
                 ...
                 className={`relative px-2 py-3 text-[11px] sm:text-[12px] font-black rounded-xl transition-all duration-200 flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 uppercase tracking-wide ${
                   isActive ? 'text-indigo-700' : 'text-zinc-500 hover:bg-zinc-300/50'
                 }`}
               >
                 {isActive && (
                 ...
                 <span className="relative z-10 flex items-center gap-1.5"><Icon size={16} strokeWidth={3} /> {tab.label}</span>
               </button>
            )
          })}
        </div>
          ...
          <div className="relative bg-white rounded-t-[2rem] sm:rounded-[2rem] w-full max-w-md shadow-2xl animate-in slide-in-from-bottom-10 sm:zoom-in-95 flex flex-col max-h-[90vh] border-t-2 sm:border-2 border-zinc-100">
            ...
            <div className="px-6 py-5 border-b-2 border-zinc-100 flex justify-between items-center bg-white rounded-t-[2rem]">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-indigo-50 border-2 border-indigo-100 flex items-center justify-center text-indigo-600 shadow-sm shrink-0">
                  <Info size={20} strokeWidth={3}/>
                </div>
                <h2 className="text-[18px] font-black text-zinc-900 tracking-tight">{t.info.title}</h2>
              </div>
              <button onClick={() => setIsInfoModalOpen(false)} className="w-8 h-8 flex items-center justify-center bg-zinc-100 hover:bg-zinc-200 border-2 border-zinc-200 rounded-full text-zinc-500 transition-colors">
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar space-y-4">
              ...
              <div className="flex items-center gap-4 p-3 bg-white rounded-xl border-2 border-zinc-200">
                 <div className="w-10 h-10 bg-amber-50 rounded-lg flex items-center justify-center text-amber-500 shrink-0"><Hash size={18} strokeWidth={2.5}/></div>
                 <div className="min-w-0"><p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">{t.code}</p><p className="font-mono font-black text-[16px] text-zinc-800 tracking-widest truncate">{classData.joinCode}</p></div>
              </div>
