...
const AIQuestionCard = ({ q, idx, onRemove }: { q: AIQuestion, idx: number, onRemove: (id: string) => void }) => {
  ...
  return (
    <div className="bg-white p-3.5 md:p-5 rounded-2xl md:rounded-3xl border border-slate-200 shadow-[0_2px_8px_rgba(0,0,0,0.02)] hover:shadow-md hover:border-amber-200 transition-all duration-300 animate-in fade-in slide-in-from-bottom-4 relative group">
      ...
      <div className="flex justify-between items-center gap-2 mb-3 md:mb-5 md:pb-4 md:border-b border-slate-100">
        <div className="flex items-center flex-wrap gap-1.5 flex-1 min-w-0">
          <span className="bg-amber-50 text-amber-700 text-[10px] md:text-[11px] font-black px-2 md:px-3 py-1 md:py-1.5 rounded-lg md:rounded-xl uppercase tracking-widest flex items-center gap-1 md:gap-1.5 border border-amber-100/50 shrink-0">
            Q{idx + 1}
          </span>
          ...
          <span className="hidden sm:inline-flex bg-slate-900 text-white text-[11px] font-bold px-3 py-1.5 rounded-xl uppercase tracking-widest shadow-sm shrink-0">{q.uiDifficulty}</span>
        </div>
        ...
        <div className="flex items-center bg-slate-50 md:bg-transparent border border-slate-200 md:border-transparent rounded-lg md:rounded-none p-0.5 md:p-0 shrink-0">
          ...
          {getText(q.explanation).trim().length > 0 && (
            <>
              ...
              <button onClick={() => setShowExplanation(!showExplanation)} className={`p-1.5 md:p-2 rounded-md md:rounded-xl transition-colors ${showExplanation ? 'bg-amber-100 text-amber-600' : 'text-slate-400 hover:text-amber-600 hover:bg-white md:hover:bg-amber-50'}`}><BookOpen size={16} className="md:w-[18px] md:h-[18px]" /></button>
            </>
          )}
      ...
      {/* QUESTION TEXT */}
      <p className="font-semibold text-[14.5px] md:text-[15.5px] text-slate-900 mb-3 md:mb-6 leading-snug md:leading-relaxed"><FormattedText text={getText(q.question)} /></p>
      ...
      {showOptions && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 md:gap-3 mb-1">
          {Object.entries(q.options).map(([key, value]) => {
            ...
            return (
              <div key={key} className={`flex items-start p-2.5 md:p-3.5 rounded-xl md:rounded-2xl border transition-all ${isCorrect ? 'bg-amber-50 border-amber-300/50' : 'bg-slate-50/50 md:bg-white border-slate-100 md:border-slate-200 hover:border-slate-300'}`}>
                <div className={`w-6 h-6 md:w-7 md:h-7 rounded-md md:rounded-lg flex items-center justify-center text-[11px] md:text-[12px] font-black mr-2.5 md:mr-3 shrink-0 transition-colors ${isCorrect ? 'bg-amber-500 text-white shadow-sm shadow-amber-500/20' : 'bg-white md:bg-slate-100 text-slate-400 md:text-slate-500 border border-slate-200 md:border-none'}`}>{key}</div>
                <div className={`text-[13px] md:text-[14.5px] font-medium pt-0.5 break-words overflow-hidden ${isCorrect ? 'text-amber-950' : 'text-slate-700'}`}><FormattedText text={getText(value)} /></div>
              </div>
            );
          })}
        </div>
      ...
      {showExplanation && getText(q.explanation).trim().length > 0 && (
        <div className="bg-amber-50/50 border border-amber-100/60 p-3.5 md:p-4.5 rounded-xl md:rounded-2xl mt-3 animate-in fade-in slide-in-from-top-2">
          <p className="text-[10px] md:text-[11px] font-black text-amber-500 uppercase tracking-widest mb-1.5 md:mb-2 flex items-center gap-1.5"><Sparkles size={12} className="md:w-[14px] md:h-[14px]" /> AI Yechim Mantiqi</p>
  ...
  return (
    <div className="flex h-[100dvh] bg-[#FAFAFA] overflow-hidden font-sans selection:bg-amber-100 selection:text-amber-900">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-white/90 backdrop-blur-xl border-b border-slate-200 z-[100000] flex items-center justify-between px-4 shadow-sm">
        ...
        <button onClick={() => setIsSidebarOpen(true)} className="p-2 -mr-2 bg-amber-50 text-amber-600 rounded-lg shadow-sm border border-amber-100/50"><Menu size={20} /></button>
      </div>
          ...
          <aside className={`fixed lg:relative top-16 lg:top-0 left-0 w-full sm:w-[400px] h-[calc(100dvh-64px)] lg:h-[100dvh] bg-white border-r border-slate-200 shadow-2xl lg:shadow-none z-[99998] lg:z-10 transition-transform duration-300 ease-in-out flex flex-col ${isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
            <div className="flex justify-between items-center p-4 md:p-5 border-b border-slate-100 bg-white shrink-0 sticky top-0 z-10">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-bold text-[16px] text-slate-900 tracking-tight flex items-center gap-2"><Sparkles size={18} className="text-amber-500"/> Ixtisos. Dastur</h2>
              </div>
              <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden p-2 bg-slate-50 text-slate-500 hover:bg-slate-100 hover:text-slate-900 rounded-full border border-slate-200 transition-colors"><X size={18} strokeWidth={2.5}/></button>
            </div>
        ...
        <div className="hidden lg:flex sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-slate-200/80 px-8 py-4 justify-between items-center shadow-sm">
          <div className="flex items-center gap-4">
            ...
            {generatedQuestions.length > 0 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 rounded-full border border-amber-200/60">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleInitiatePublish} disabled={isPublishing || isGenerating || generatedQuestions.length === 0} className="bg-slate-900 hover:bg-slate-800 text-white px-5 py-2.5 rounded-xl font-bold shadow-md shadow-slate-900/10 transition-all flex items-center gap-2 active:scale-95 disabled:opacity-50 text-[14px]">
               <CheckCircle2 size={18} /> Nashr qilish
            </button>
          </div>
        </div>

        <div className="max-w-[800px] mx-auto p-4 md:p-8 space-y-4 md:space-y-6">
          {generatedQuestions.length === 0 && !isGenerating ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="h-[60vh] flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-200 rounded-[2rem] bg-white lg:mt-6 p-6 text-center shadow-sm mx-2 lg:mx-0">
              <div className="w-16 h-16 bg-amber-50 rounded-2xl flex items-center justify-center mb-5 border border-amber-100"><Sparkles size={28} className="text-amber-500" /></div>
              ...
              <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden mt-8 px-6 py-3 bg-amber-500 text-white font-bold rounded-xl shadow-md active:scale-95">Parametrlarni tanlash</button>
            </motion.div>
          ) : (
      ...
      {mounted && !isSidebarOpen && generatedQuestions.length > 0 && createPortal(
        <div className="lg:hidden fixed bottom-6 left-0 right-0 px-4 flex justify-between gap-3 z-30 animate-in slide-in-from-bottom-10">
          <button onClick={() => setIsSidebarOpen(true)} className="w-14 h-14 bg-amber-500 text-white rounded-2xl shadow-xl shadow-amber-500/20 flex items-center justify-center active:scale-95 transition-transform shrink-0 border border-amber-400">
                  ...
                  <div className="flex-1 overflow-y-auto p-5 custom-scrollbar bg-[#FAFAFA]">
                    <div className="grid grid-cols-2 gap-3">
                      {availableClasses.map(c => (
                        <button key={c} onClick={() => handleClassSelect(c)} className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center justify-center gap-3 active:scale-[0.98] ${selectedClass === c ? 'bg-amber-50 border-amber-500 text-amber-700 shadow-md ring-2 ring-amber-500/20' : 'bg-white border-slate-200 text-slate-700 hover:border-amber-400 hover:shadow-sm'}`}>
                          <GraduationCap size={28} className={selectedClass === c ? "text-amber-500" : "text-slate-400"}/>
                  ...
                  <div className="flex-1 overflow-y-auto p-5 custom-scrollbar bg-[#FAFAFA]">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {availableSubjects.map((s: string) => (
                        <button key={s} onClick={() => handleSubjectSelect(s)} className={`p-4 rounded-2xl border-2 transition-all flex items-center gap-4 active:scale-[0.98] ${selectedSubject === s ? 'bg-amber-50 border-amber-500 text-amber-700 shadow-md ring-2 ring-amber-500/20' : 'bg-white border-slate-200 text-slate-700 hover:border-amber-400 hover:shadow-sm'}`}>
                          <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${selectedSubject === s ? 'bg-amber-500 text-white' : 'bg-slate-50 border border-slate-200 text-slate-400'}`}>
                            <BookOpen size={20} />
                          </div>
                  ...
                  <div className="p-5 md:p-6 border-b border-slate-100 flex flex-col gap-4 bg-white shrink-0">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-11 pr-4 py-3.5 rounded-2xl border border-slate-200 bg-slate-50 focus:bg-white focus:outline-none focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all text-[14px] font-bold text-slate-800 placeholder:text-slate-400"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-5 md:p-6 custom-scrollbar bg-[#FAFAFA]">
                    {syllabusData.chapters.map((chapter: any) => {
                      ...
                      return (
                        <div key={chapter.index} className="mb-8 last:mb-0 animate-in fade-in">
                          <h4 className="text-[12px] font-black text-amber-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                            ...
                            {filteredSubtopics.map((sub: any) => {
                              ...
                              return (
                                <button 
                                  ...
                                  className={`text-left p-4 rounded-2xl border transition-all duration-200 flex flex-col justify-between h-full min-h-[90px] active:scale-[0.98] ${isSelected ? 'bg-amber-500 border-amber-500 text-white shadow-md' : 'bg-white border-slate-200 hover:border-amber-400 hover:shadow-sm'}`}
                                >
                                  <span className={`text-[13.5px] font-bold leading-snug ${isSelected ? 'text-white' : 'text-slate-800'}`}>{sub.name}</span>
                                  <span className={`text-[10px] font-black uppercase tracking-widest mt-4 ${isSelected ? 'text-amber-200' : 'text-slate-400'}`}>Mavzu {sub.index}</span>
                                </button>
                              );
                            })}
                          </div>
                    ...
                    {syllabusData.chapters.every((c: any) => c.subtopics.filter((s: any) => s.name.toLowerCase().includes(searchQuery.toLowerCase())).length === 0) && (
                      <div className="flex flex-col items-center justify-center py-16 text-slate-400">
                        <Search size={40} className="mb-4 opacity-30 text-amber-500" />
            ...
            {isTitleModalOpen && (
              <div className="fixed inset-0 z-[100000] flex items-center justify-center p-4">
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="relative bg-white rounded-3xl p-6 md:p-8 w-full max-w-md shadow-2xl border border-slate-100 z-10">
                  <div className="w-16 h-16 bg-amber-50 rounded-2xl flex items-center justify-center mb-5 border border-amber-100"><BookOpen size={28} strokeWidth={2.5} className="text-amber-600" /></div>
                  ...
                  <input type="text" value={testTitle} onChange={e => setTestTitle(e.target.value)} className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-[14px] font-black text-slate-900 outline-none focus:bg-white focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all mb-8 placeholder:font-medium placeholder:text-slate-400" autoFocus placeholder="Masalan: Olimpiada tayyorgarligi"/>
