    ...
    <AnimatePresence>
      {isVisible && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-[320px] md:max-w-md bg-white/90 backdrop-blur-2xl rounded-[1.5rem] md:rounded-3xl shadow-2xl p-6 md:p-8 flex flex-col items-center justify-center overflow-hidden">
            ...
            <div className="relative mb-6 md:mb-8 mt-2 md:mt-4">
              ...
              <div className="relative w-20 h-20 md:w-24 md:h-24 bg-white/80 backdrop-blur-md rounded-2xl md:rounded-3xl flex items-center justify-center shadow-xl">
...
const AIQuestionCard = ({ q, idx, onRemove }: { q: AIQuestion, idx: number, onRemove: (id: string) => void }) => {
  ...
  return (
    <div className="bg-white p-3.5 md:p-5 rounded-[1.25rem] md:rounded-3xl border border-slate-200 shadow-[0_2px_8px_rgba(0,0,0,0.02)] hover:shadow-md hover:border-indigo-200 transition-all duration-300 animate-in fade-in slide-in-from-bottom-4 relative group">
      <div className="flex justify-between items-start gap-2 mb-3 md:mb-5 pb-2.5 md:pb-4 border-b border-slate-100">
        <div className="flex items-center flex-wrap gap-1.5 md:gap-2 flex-1 min-w-0">
          ...
          <span className="bg-slate-50 text-slate-600 text-[9px] md:text-[11px] font-bold px-2 md:px-3 py-1 md:py-1.5 rounded-md md:rounded-xl uppercase tracking-widest flex items-center border border-slate-200/60 max-w-full min-w-0">
      ...
      {showOptions && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 md:gap-3 mb-1 md:mb-2">
          {Object.entries(q.options).map(([key, value]) => {
            ...
            return (
              <div key={key} className={`flex items-start p-2 md:p-3.5 rounded-xl md:rounded-2xl border-2 transition-all ${isCorrect ? 'bg-indigo-50/40 border-indigo-500/30' : 'bg-slate-50/50 md:bg-white border-slate-100'}`}>
                <div className={`w-5 h-5 md:w-7 md:h-7 rounded-md md:rounded-lg flex items-center justify-center text-[10px] md:text-[12px] font-black mr-2 md:mr-3 shrink-0 mt-0.5 transition-colors ${isCorrect ? 'bg-indigo-600 text-white shadow-sm shadow-indigo-600/20' : 'bg-white md:bg-slate-100 text-slate-400 md:text-slate-500 border border-slate-200 md:border-none'}`}>{key}</div>
      ...
      {getText(q.explanation).trim().length > 0 && (
        <div className="mt-2.5 md:mt-4 pt-2.5 md:pt-4 border-t border-slate-50 flex justify-start">
          <button onClick={() => setShowExplanation(!showExplanation)} className={`flex items-center gap-1.5 md:gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl text-[11px] md:text-[13px] font-bold transition-all duration-300 ${showExplanation ? 'bg-indigo-100 text-indigo-700 shadow-inner' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100 hover:shadow-sm'}`}>
            <Sparkles size={12} className={`md:w-4 md:h-4 ${showExplanation ? "text-indigo-500" : "text-indigo-400"}`} />
      ...
      {showExplanation && getText(q.explanation).trim().length > 0 && (
        <div className="bg-slate-50 border border-slate-100 p-2.5 md:p-4.5 rounded-xl md:rounded-2xl mt-2.5 md:mt-4 animate-in fade-in slide-in-from-top-2">
          <p className="text-[9px] md:text-[11px] font-black text-indigo-400 uppercase tracking-widest mb-1.5 md:mb-2 flex items-center gap-1 md:gap-1.5"><Sparkles size={10} className="text-indigo-400 md:w-[14px] md:h-[14px]" /> AI Yechim Mantiqi</p>
  ...
  return (
    <div className="flex h-[100dvh] bg-[#FAFAFA] overflow-hidden font-sans selection:bg-indigo-100 selection:text-indigo-900">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-[60px] bg-white/90 backdrop-blur-xl border-b border-slate-200 z-[100000] flex items-center justify-between px-3 shadow-sm">
        ...
        <button onClick={() => setIsSidebarOpen(true)} className="p-2 -mr-1 bg-indigo-50 text-indigo-600 rounded-lg shadow-sm border border-indigo-100/50"><Menu size={18} /></button>
      </div>
          ...
          <aside className={`fixed lg:relative top-[60px] lg:top-0 left-0 w-full sm:w-[400px] h-[calc(100dvh-60px)] lg:h-[100dvh] bg-white border-r border-slate-200 shadow-2xl lg:shadow-none z-[99998] lg:z-10 transition-transform duration-300 ease-in-out flex flex-col ${isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
            ...
            <div className="flex justify-between items-center p-3 md:p-5 border-b border-slate-100 bg-white shrink-0 sticky top-0 z-10">
              <div className="flex items-center gap-2 md:gap-3">
                ...
                <h2 className="font-bold text-[14px] md:text-[16px] text-slate-900 tracking-tight flex items-center gap-2">
                  <Layers size={16} className="text-indigo-600 md:w-[18px] md:h-[18px]"/> Maktab Dasturi
                </h2>
              </div>
              
              <button 
                onClick={() => setIsSidebarOpen(false)} 
                className="lg:hidden p-1.5 bg-slate-50 text-slate-500 hover:bg-slate-100 hover:text-slate-900 rounded-full border border-slate-200 transition-colors"
              >
        ...
        <div className="hidden lg:flex sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-slate-200/80 px-8 py-3 justify-between items-center shadow-sm">
          <div className="flex items-center gap-4">
            ...
            {generatedQuestions.length > 0 && (
              <div className="flex items-center gap-1.5 px-3 py-1 bg-indigo-50 rounded-full border border-indigo-200/60">
        ...
        <div className="max-w-[800px] mx-auto p-3 md:p-8 space-y-4 md:space-y-6">
          {generatedQuestions.length === 0 && !isGenerating ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="h-[60vh] flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-200 rounded-[1.5rem] bg-white lg:mt-6 p-4 md:p-6 text-center shadow-sm mx-1 lg:mx-0">
              <div className="w-12 h-12 md:w-16 md:h-16 bg-indigo-50 rounded-xl md:rounded-2xl flex items-center justify-center mb-4 md:mb-5 border border-indigo-100"><Sparkles size={24} className="text-indigo-500 md:w-7 md:h-7" /></div>
              ...
              <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden mt-6 px-6 py-3 bg-indigo-600 text-white font-bold rounded-xl shadow-md active:scale-95 text-[13px]">Parametrlarni tanlash</button>
            </motion.div>
          ) : (
      ...
      {mounted && !isSidebarOpen && generatedQuestions.length > 0 && createPortal(
        <div className="lg:hidden fixed bottom-5 left-0 right-0 px-3 flex justify-between gap-2 z-[100] animate-in slide-in-from-bottom-10">
          <button 
            ...
            className="w-12 h-[46px] bg-indigo-600 text-white rounded-[1rem] shadow-xl shadow-indigo-600/20 flex items-center justify-center active:scale-95 transition-transform shrink-0 border border-indigo-500"
          >
          ...
          <button 
            ...
            className="flex-1 bg-indigo-50 text-indigo-700 rounded-[1rem] shadow-lg shadow-indigo-600/10 font-bold flex items-center justify-center gap-1.5 active:scale-95 transition-transform text-[11px] h-[46px] border border-indigo-200/50"
          >
          ...
          <button 
            ...
            className="flex-1 bg-slate-900 text-white rounded-[1rem] shadow-xl shadow-slate-900/20 font-bold flex items-center justify-center gap-1.5 active:scale-95 transition-transform text-[11px] h-[46px] border border-slate-800"
          >
                  ...
                  <div className="flex-1 overflow-y-auto p-4 md:p-5 custom-scrollbar bg-[#FAFAFA]">
                    <div className="grid grid-cols-2 gap-2.5 md:gap-3">
                      {availableClasses.map(c => (
                        <button key={c} onClick={() => handleClassSelect(c)} className={`p-3 md:p-4 rounded-2xl border-2 transition-all flex flex-col items-center justify-center gap-2 md:gap-3 active:scale-[0.98] ${selectedClass === c ? 'bg-indigo-50 border-indigo-500 text-indigo-700 shadow-md ring-2 ring-indigo-500/20' : 'bg-white border-slate-200 text-slate-700 hover:border-indigo-400 hover:shadow-sm'}`}>
                          <GraduationCap size={24} className={`md:w-7 md:h-7 ${selectedClass === c ? "text-indigo-500" : "text-slate-400"}`}/>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 md:p-5 custom-scrollbar bg-[#FAFAFA]">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 md:gap-3">
                      {availableSubjects.map((s: string) => (
                        <button key={s} onClick={() => handleSubjectSelect(s)} className={`p-3 md:p-4 rounded-xl md:rounded-2xl border-2 transition-all flex items-center gap-3 md:gap-4 active:scale-[0.98] ${selectedSubject === s ? 'bg-indigo-50 border-indigo-500 text-indigo-700 shadow-md ring-2 ring-indigo-500/20' : 'bg-white border-slate-200 text-slate-700 hover:border-indigo-400 hover:shadow-sm'}`}>
                          <div className={`w-10 h-10 md:w-12 md:h-12 rounded-lg md:rounded-xl flex items-center justify-center shrink-0 ${selectedSubject === s ? 'bg-indigo-600 text-white' : 'bg-slate-50 border border-slate-200 text-slate-400'}`}>
                  ...
                  <div className="p-4 md:p-6 border-b border-slate-100 flex flex-col gap-3 md:gap-4 bg-white shrink-0">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-10 pr-4 py-2.5 md:py-3.5 rounded-[1rem] md:rounded-2xl border border-slate-200 bg-slate-50 focus:bg-white focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all text-[13px] md:text-[14px] font-bold text-slate-800 placeholder:text-slate-400"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar bg-[#FAFAFA]">
                    {syllabusData.chapters.map((chapter: any) => {
                      ...
                      return (
                        <div key={chapter.index} className="mb-6 md:mb-8 last:mb-0 animate-in fade-in">
                          <h4 className="text-[10px] md:text-[12px] font-black text-indigo-600 uppercase tracking-widest mb-3 md:mb-4 flex items-center gap-1.5 md:gap-2">
                            ...
                            {filteredSubtopics.map((sub: any) => {
                              ...
                              return (
                                <button 
                                  ...
                                  className={`text-left p-3.5 md:p-4 rounded-[1rem] md:rounded-2xl border transition-all duration-200 flex flex-col justify-between h-full min-h-[80px] md:min-h-[90px] active:scale-[0.98] ${isSelected ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' : 'bg-white border-slate-200 hover:border-indigo-400 hover:shadow-sm'}`}
                                >
                                  <span className={`text-[12px] md:text-[13.5px] font-bold leading-snug ${isSelected ? 'text-white' : 'text-slate-800'}`}>{sub.name}</span>
                                  <span className={`text-[9px] md:text-[10px] font-black uppercase tracking-widest mt-3 md:mt-4 ${isSelected ? 'text-indigo-200' : 'text-slate-400'}`}>Mavzu {sub.index}</span>
                                </button>
                              );
                            })}
                          </div>
                    ...
                    {syllabusData.chapters.every((c: any) => c.subtopics.filter((s: any) => s.name.toLowerCase().includes(searchQuery.toLowerCase())).length === 0) && (
                      <div className="flex flex-col items-center justify-center py-12 md:py-16 text-slate-400">
                        <Search size={32} className="mb-3 md:mb-4 opacity-30 text-indigo-500 md:w-10 md:h-10" />
