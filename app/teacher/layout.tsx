...
const SIDEBAR_TRANSLATIONS = {
  uz: {
    ...
    menu: { overview: "Umumiy", library: "Kutubxona", classes: "Sinflar", onlineLibrary: "Onlayn Kutubxona", analytics: "AI Tahlilchi", profile: "Profil" },
  ...
  en: {
    ...
    menu: { overview: "Overview", library: "My Library", classes: "My Classes", onlineLibrary: "Online Library", analytics: "AI Analytics", profile: "Profile" },
  ...
  ru: {
    ...
    menu: { overview: "Обзор", library: "Библиотека", classes: "Классы", onlineLibrary: "Онлайн Библиотека", analytics: "ИИ Аналитика", profile: "Профиль" },
        ...
        {collapsed ? (
          ...
          <>
            <span className="flex items-center gap-2">
              <span className="w-5 h-5 rounded bg-slate-100 flex items-center justify-center text-[9px] border border-slate-200 text-slate-700">{lang}</span>
...
export default function TeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  const [avatarError, setAvatarError] = useState(false); // 🟢 Safe at the top level
  ...
  if (loading || !isAuthorized) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#FAFAFA] gap-4">
        <div className="w-12 h-12 bg-white rounded-2xl border border-slate-200 shadow-sm flex items-center justify-center">
          <BookOpen size={24} className="text-indigo-600 animate-pulse" />
        </div>
  ...
  // 🟢 EXTRACTED RENDER FUNCTION (Prevents component destruction)
          ...
          <Link href="/teacher/profile" className={`flex items-center gap-3 flex-1 group rounded-xl p-2 transition-colors ${!collapsed && '-ml-2 hover:bg-slate-50'}`}>
            <div className="relative shrink-0">
              <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-black text-[16px] shadow-sm border-2 border-white group-hover:border-indigo-100 transition-colors bg-gradient-to-tr from-indigo-600 to-violet-500 overflow-hidden">
                {user?.photoURL && !avatarError ? (
                  <img src={user.photoURL} alt={user?.displayName || "User"} className="w-full h-full object-cover" onError={() => setAvatarError(true)} />
                ) : (
            ...
            {!collapsed && (
              <div className="overflow-hidden text-left">
                <p className="text-[14px] font-bold text-slate-900 truncate group-hover:text-indigo-600 transition-colors">{user?.displayName || 'Teacher'}</p>
                <p className="text-[12px] font-medium text-slate-500 truncate">{t.menu.profile}</p>
              </div>
            )}
          </Link>

          <button onClick={() => setShowLogoutModal(true)} className={`text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-colors ${collapsed ? 'p-2 mt-2 bg-slate-50 border border-slate-100' : 'p-2.5'}`} title={t.signOut}>
            <LogOut size={18} />
          </button>
        </div>
      </div>
    ...
    <TeacherLanguageContext.Provider value={{ lang, setLang }}>
      <div className="min-h-[100dvh] bg-[#FAFAFA] font-sans selection:bg-indigo-100 selection:text-indigo-900">
        ...
        <header className="md:hidden bg-white/80 backdrop-blur-md text-slate-900 p-3.5 flex items-center justify-between sticky top-0 z-30 border-b border-slate-200/80 shadow-sm">
          <div className="flex items-center gap-3">
            <button onClick={() => setMobileMenuOpen(true)} className="p-2 -ml-2 text-slate-500 hover:bg-slate-100 rounded-lg"><Menu size={22} /></button>
          ...
          <div className="flex items-center gap-2"><div className="text-slate-500 flex items-center justify-center p-1"><NotificationBell /></div></div>
        </header>
        ...
        {/* 🖥️ DESKTOP SIDEBAR */}
        <aside className={`hidden md:block fixed top-0 left-0 h-full bg-white z-20 border-r border-slate-200 shadow-[4px_0_24px_-12px_rgba(0,0,0,0.05)] transition-all duration-300 ease-in-out ${isCollapsed ? 'w-20' : 'w-[280px]'}`}>
